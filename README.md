...
| **Curated PFAS site narratives** — notable Michigan PFAS sites | ✅ embedded reference | Hand-researched, source-cited narratives in `app/pfas_narratives.py` (Wurtsmith AFB / Oscoda area, Wolverine World Wide tannery + House Street disposal, Parchment/Crown Vantage drinking water) that add history and **documented peak concentrations** on top of the terse live MPART feed. Attached to the matching live MPART site/AOI records at load time (county + name-token match) — **not** as duplicate markers — by `apply_pfas_narratives`; the live feed stays authoritative for location/status/contact and the popup keeps its MPART site-investigation link. Every figure states its **medium and the criterion that applies to that medium** (groundwater vs Michigan's Part 201 cleanup criteria of 16 ppt PFOS / 8 ppt PFOA; drinking water vs the health advisory in effect when sampled) — a groundwater reading is never compared to the 4 ppt drinking-water MCL. Nothing is fabricated: unverifiable figures are omitted. |
